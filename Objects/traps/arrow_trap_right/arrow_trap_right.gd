...
#          <applies_to>.self</applies_to>
#          <arguments>
#            <argument kind="STRING">if (not InLevel.is_room("r_level_editor")):

#        # DY:  calculate how far to motion detect
#        x_act = position.x+16
#        n = 100
#        while (not gml.collision_point(x_act, position.y+8, "solid", 0, 0) &amp&amp n > 0):
...
#        obj = gml.instance_create(position.x+16, position.y, "arrow_trap_test")
