...
def search_gml_functions(passed_text):
    gml_functions = re.findall('([a-z]+_[a-z0-9]+)\(', passed_text)
...
def write_all_gml_functions_to_file(list):
    file_path = r'C:\Users\Jesse\Desktop\H&C2\programming\Spelunky Godot Porting Scripts\GMLFunctionsDumpAllVariables.txt'
    with open(file_path, 'w') as file:
         for line in list:
                file.write(f"{line}\n")
