...
#
	...
	if (LevelGeneration.scr_get_room_x(x) == global.start_room_x and LevelGeneration.scr_get_room_y(y) == global.start_room_y): # DY:  start room
		...
		str_temp = "000000000000000000000000000000000000000000010010000011001100011101110111BBBB111"
			...
			if (not InLevel.is_in_shop(block_instance.position.x, block_instance.position.y)):
				...
				if (not InLevel.is_in_shop(block_instance.position.x, block_instance.position.y) and
					...
					if (gml.collision_point(block_instance.position.x+16, block_instance.position.y, "solid", 0, 0) and not
						gml.collision_rectangle(block_instance.position.x-32, block_instance.position.y, block_instance.position.x-1,  block_instance.position.y+15, "solid", 0, 0, block_instance)):
						...
						gml.collision_rectangle(block_instance.position.x+16, block_instance.position.y, block_instance.position.x+48,  block_instance.position.y+15, "solid", 0, 0, block_instance)):
		...
		for solid_instance in all_solids:
			...
			if (not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y)):
				...
				if (solid_instance.position.y > 32 and gml.collision_point(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and global.gen_market_entrance and not global.made_market_entrance):
					...
					not gml.collision_rectangle(solid_instance.position.x, solid_instance.position.y-16, solid_instance.position.x+15,  solid_instance.position.y-1, "enemy", 0, 0, solid_instance) and
				...
				if (LevelGeneration.scr_get_room_x(solid_instance.position.x) != global.start_room_x or LevelGeneration.scr_get_room_y(solid_instance.position.y-16) != global.start_room_y and
					...
					if (solid_instance.position.y > 16 and not gml.collision_point(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)):
						...
						if (global.gen_tomb_lord and
							...
							not gml.collision_rectangle(solid_instance.position.x, solid_instance.position.y-32, solid_instance.position.x+32,  solid_instance.position.y-1, "solid", 0, 0, solid_instance) and
							gml.rand(1,40) == 1):
