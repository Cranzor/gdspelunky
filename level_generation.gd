...
#***********************************************************************************/
	...
	var col_stuff = true
	if (not gml.collision_point_test(x, y-16, 'solid', 0, 0) and #same as above for here
		not gml.collision_point_test(x, y-1, 'treasure', 0, 0) and
		not gml.collision_point_test(x, y-8, 'chest', 0, 0) and
		not gml.collision_point_test(x, y-8, 'spikes', 0, 0) and
		not gml.collision_point_test(x, y-8, 'entrance', 0, 0) and
		not gml.collision_point_test(x, y-8, 'exit', 0, 0)):
	...
	if (not col_stuff and
		gml.collision_point_test(x, y-32, 'solid', 0, 0) and
		(gml.collision_point_test(x-16, y-16, 'solid', 0, 0) or gml.collision_point_test(x+16, y-16, 'solid', 0, 0) or gml.collision_point_test(x-16, y-16, 'block', 0, 0) or gml.collision_point_test(x+16, y-16, 'block', 0, 0))):
		...
		elif (not damsel and randi_range(1,8) == 1 and not gml.collision_point_test(x+8, y-8, 'water', 0, 0)):
	 ...
	 # tunnel
		...
		(gml.collision_point_test(x-16, y-16, 'solid', 0, 0) and gml.collision_point_test(x+16, y-16, 'solid', 0, 0))):
	 ...
	 # normal
	elif (not gml.collision_point_test(x, y-16, 'solid', 0, 0) and
		not gml.collision_point_test(x, y-8, 'chest', 0, 0) and
		not gml.collision_point_test(x, y-8, 'spikes', 0, 0) and
		not gml.collision_point_test(x, y-8, 'entrance', 0, 0) and
		not gml.collision_point_test(x, y-8, 'exit', 0, 0)):
...
#***********************************************************************************/
	...
	for treasure in all_treasure:
		...
		if (gml.collision_point_test(treasure.position.x, treasure.position.y, 'solid', 0, 0)):
			var obj
...
func scr_room_gen(x, y):
	...
	for j in range(0, 8):
		...
		for i in range(1, 11):
			...
			var ypos = y + j*16
			#--------------------------------------------------------------------------------------
			if (tile == "1" and not gml.collision_point_test(xpos, ypos, 'solid', 0, 0)): #arguments were originally (xpos, ypos, solid, 0, 0). almost every other time seems to use 0 or -1 instead of "solid" --- changed it back because not sure why I changed it in the first place
			...
			elif (tile == "2" and randi_range(1,2) == 1 and not gml.collision_point_test(xpos, ypos, 'solid', 0, 0)):
			...
			elif (tile == "." and not gml.collision_point_test(xpos, ypos, 'solid', 0, 0)):
...
#
	...
	if (global.level_type == 0):
			...
			if (not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y) and solid_instance.position.y > 16):
				...
				if (scr_get_room_x(solid_instance.position.x) != global.start_room_x or scr_get_room_y(solid_instance.position.y-16) != global.start_room_y):
					...
					if (solid_instance.position.y < gml.room_height() - 64 and 
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, 'solid', 0, 0) and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+32, 'solid', 0, 0) and
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, 'water', 0, 0) and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+32, 'water', 0, 0) and
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, 'enemy', 0, 0)):
					
						if (global.gen_giant_spider and
							not global.giant_spider and
							not gml.collision_point_test(solid_instance.position.x+16, solid_instance.position.y+16, 'solid', 0, 0) and
							not gml.collision_point_test(solid_instance.position.x+16, solid_instance.position.y+32, 'solid', 0, 0) and
							randi_range(1,40) == 1):
					...
					if (not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, 'solid', 0, 0)):
		...
		if (global.gen_udjat_eye and not global.locked_chest):
			...
			for exit_instance in all_exits:
				...
				if (not gml.collision_point_test(exit_instance.position.x-8, exit_instance.position.y, 'solid', 0, 0) and
					not gml.collision_point_test(exit_instance.position.x-8, exit_instance.position.y+15, 'treasure', 0, 0) and
					not gml.collision_point_test(exit_instance.position.x-8, exit_instance.position.y+8, 'chest', 0, 0) and
					not gml.collision_point_test(exit_instance.position.x-8, exit_instance.position.y+8, 'spikes', 0, 0)):
				
					gml.instance_create(exit_instance.position.x-8, exit_instance.position.y+8, locked_chest)
					global.Locked_chest = true
					break
				
				elif (not gml.collision_point_test(exit_instance.position.x+8, exit_instance.position.y, 'solid', 0, 0) and
						 not gml.collision_point_test(exit_instance.position.x+8, exit_instance.position.y+15, 'treasure', 0, 0) and
						 not gml.collision_point_test(exit_instance.position.x+8, exit_instance.position.y+8, 'chest', 0, 0) and
						 not gml.collision_point_test(exit_instance.position.x+8, exit_instance.position.y+8, 'spikes', 0, 0)):
		...
		if (gml.instance_exists('locked_chest')):
			...
			while (n < 8 and global.key == false):
				...
				for treasure_instance in all_treasure:
					...
					if (randi_range(1,8) <= 1 and not gml.collision_point_test(treasure_instance.position.x, treasure_instance.position.y, 'solid', 0, 0) and global.key == false):
			...
			if (not global.key):
				...
				for treasure_instance in all_treasure:
					...
					if (not gml.collision_point_test(treasure_instance.position.x, treasure_instance.position.y, "solid", 0, 0)):
		...
		for block_instance in all_blocks:
			...
			if (not InLevel.is_in_shop(block_instance.position.x, block_instance.position.y)):
				...
				if (not InLevel.is_in_shop(block_instance.position.x, block_instance.position.y) and
					...
					if (gml.collision_point_test(block_instance.position.x+16, block_instance.position.y, "solid", 0, 0) and not
						gml.collision_rectangle_test(block_instance.position.x-32, block_instance.position.y, block_instance.position.x-1, block_instance.position.y+15, "solid", 0, 0)): #--- originally written as collision_point,
					...
					elif (gml.collision_point_test(block_instance.position.x-16, block_instance.position.y, "solid", 0, 0) and not
						gml.collision_rectangle_test(block_instance.position.x+16, block_instance.position.y, block_instance.position.x+48, block_instance.position.y+15, "solid", 0, 0)): #--- same as above
		...
		if (global.cemetary):
			...
			for lush_instance in all_lushes:
				...
				# generate graves
				if (not gml.collision_point_test(lush_instance.position.x, lush_instance.position.y-16, "solid", 0, 0) and
					not gml.collision_point_test(lush_instance.position.x, lush_instance.position.y-16, "entrance", 0, 0) and
					not gml.collision_point_test(lush_instance.position.x, lush_instance.position.y-16, "exit", 0, 0) and
					...
					elif (not gml.collision_point_test(lush_instance.position.x+8, lush_instance.position.y+8, "treasure", 0, 0)):
		...
		for solid_instance in all_solids:
			...
			# bg
			if (randi_range(1,100) == 1 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)): gml.tile_add("bg_trees", 0, 0, 16, 48, solid_instance.position.x, solid_instance.position.y-32, 9005)
			
			if (not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y)):
				
				var entrance_instance = gml.get_instance('entrance') #--- Helper function
			
				if (solid_instance.position.y > 32 and gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and global.gen_market_entrance and not global.made_market_entrance):
					...
					if (obj.solid_instance.type != "Tree" and solid_instance.type != "Altar" and not obj.invincible and randi_range(1,global.market_chance) <= 1):
						...
						global.made_market_entrance = true
						 ...
						 not gml.collision_rectangle_test(solid_instance.position.x, solid_instance.position.y-32, solid_instance.position.x+15, solid_instance.position.y-1, "solid", false, true) and #--- this and below seem to be same issue as above with wrong function being called. changed to collision_rectangle
						 not gml.collision_rectangle_test(solid_instance.position.x, solid_instance.position.y-16, solid_instance.position.x+15, solid_instance.position.y-1, "enemy", 0, 0) and
						 (not gml.collision_point_test(solid_instance.position.x-16, solid_instance.position.y, "solid", 0, 0) or not gml.collision_point_test(solid_instance.position.x+16, solid_instance.position.y, "solid", 0, 0)) and
						 gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, "solid", 0, 0) and
						 not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y, "x_market", 0, 0) and
					...
					if (global.dark_level and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-32, "water", 0, 0) and randi_range(1,20) == 1):
						...
						if (gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)):
				...
				if (scr_get_room_x(solid_instance.position.x) != global.start_room_x or scr_get_room_y(solid_instance.position.y-16) != global.start_room_y):
					...
					if (solid_instance.position.y < gml.room_height() - 64 and 
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, "solid", 0, 0) and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+32, "solid", 0, 0) and
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, "water", 0, 0) and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+32, "water", 0, 0)):
					...
					if (solid_instance.position.y > 16 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y, "enemy", 0, 0) and
						not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y, "spikes", 0, 0)):
						...
						elif (not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "water", 0, 0)):
		...
		if (global.gen_market_entrance and not global.made_market_entrance):
			...
			for solid_instance in all_solids:
				...
				if (solid_instance.position.y > 32 and gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)):
		...
		for water_instance in all_water:
			...
			if (not gml.collision_point_test(water_instance.position.x, water_instance.position.y, "solid", 0, 0)):
		...
		for solid_instance in all_solids:
			...
			if (not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y)):
				...
				if (scr_get_room_x(solid_instance.position.x) != global.start_room_x or scr_get_room_y(solid_instance.position.y-16) != global.start_room_y):
					...
					elif (solid_instance.position.y > 16 and solid_instance.position.y < 592 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y)):
				...
				if (solid_instance.position.y > 16 and solid_instance.position.y < 592 and
					not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and
					not gml.collision_point_test(solid_instance.position.x+8, solid_instance.position.y-8, "enemy", 0, 0) and
					not gml.collision_point_test(solid_instance.position.x+8, solid_instance.position.y-1, "spikes", 0, 0) and
		...
		for solid_instance in all_solids:
			...
			# bg
			if (randi_range(1,100) == 1 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)): gml.tile_add("bg_statues", 0, 0, 16, 48, solid_instance.position.x, solid_instance.position.y-32, 9005)
			...
			if (not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y)):
				...
				# traps
				if (solid_instance.position.y > 32 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and global.gen_gold_entrance and not global.made_gold_entrance):
					...
					if (randi_range(1,global.gold_chance) == 1):
						...
						global.made_gold_entrance = true
						 ...
						 not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0) and
						 not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "lava", 0, 0) and
						 not gml.collision_rectangle_test(solid_instance.position.x, solid_instance.position.y-16, solid_instance.position.x+15, solid_instance.position.y-1, "enemy", 0, 0) and #--- collision_rectangle issue again
						 not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-32, "solid", 0, 0) and
						 (not gml.collision_point_test(solid_instance.position.x-16, solid_instance.position.y, "solid", 0, 0) or not gml.collision_point_test(solid_instance.position.x+16, solid_instance.position.y, "solid", 0, 0)) and
						 gml.collision_point_test(solid_instance.position.x, solid_instance.position.y+16, "solid", 0, 0) and
						 not InLevel.is_in_shop(solid_instance.position.x, solid_instance.position.y) and
						 solid_instance.position.x != 160 and solid_instance.position.x != 176 and solid_instance.position.x != 320 and solid_instance.position.x != 336 and solid_instance.position.x != 480 and solid_instance.position.x != 496):
					...
					if (randi_range(1,12) == 1 and gml.point_distance(solid_instance.position.x, solid_instance.position.y, entrance_instance.position.x, entrance_instance.position.y) > 64):
						...
						if (gml.collision_point_test(solid_instance.position.x-16, solid_instance.position.y-32, "solid", 0, 0) and gml.collision_point_test(solid_instance.position.x+16, solid_instance.position.y-32, "solid", 0, 0) and
							not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-32, "solid", 0, 0)):
				...
				if (scr_get_room_x(solid_instance.position.x) != global.start_room_x or scr_get_room_y(solid_instance.position.y-16) != global.start_room_y and
					...
					if (solid_instance.position.y > 16 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)):
					
						if (global.genTomb_lord and
							not global.Tomb_lord and
							not gml.collision_rectangle_test(solid_instance.position.x, solid_instance.position.y-32, solid_instance.position.x+32, solid_instance.position.y-1, "solid", 0, 0) and #collision_rectangle issue again
							randi_range(1,40) == 1):
		...
		if (global.gen_gold_entrance and not global.made_gold_entrance):
			...
			for solid_instance in all_solids:
				...
				if (solid_instance.position.y > 32 and not gml.collision_point_test(solid_instance.position.x, solid_instance.position.y-16, "solid", 0, 0)):
		...
		for block_instance in all_blocks:
			...
			if (not InLevel.is_in_shop(block_instance.position.x, block_instance.position.y)):
				...
				if (not InLevel.is_in_shop(block_instance.position.x, block_instance.position.y) and
					...
					if (gml.collision_point_test(block_instance.position.x+16, block_instance.position.y, "solid", 0, 0) and not
						gml.collision_rectangle_test(block_instance.position.x-32, block_instance.position.y, block_instance.position.x-1, block_instance.position.y+15, "solid", 0, 0)): #--- collision_rectangle issue again
						...
						gml.collision_rectangle_test(block_instance.position.x+16, block_instance.position.y, block_instance.position.x+48, block_instance.position.y+15, "solid", 0, 0)): #--- collision_rectangle issue again
	...
	if (global.dark_level):
		...
		for entrance_instance in all_entrances:
			...
			if (not gml.collision_point_test(entrance_instance.position.x-16, entrance_instance.position.y, "solid", 0, 0)):
			
				gml.instance_create(entrance_instance.position.x-16+8, entrance_instance.position.y+8, flare_crate)
			
			elif (not gml.collision_point_test(entrance_instance.position.x+16, entrance_instance.position.y, "solid", 0, 0)):
...
#
	...
	for brick_instance in all_bricks:
		...
		if (brick_instance.position.y == 0 or gml.collision_point_test(brick_instance.position.x, brick_instance.position.y-16, 'brick', 0, 0) or gml.collision_point_test(brick_instance.position.x, brick_instance.position.y-16, 'hard_block', 0, 0)):  brick_instance.up = true
		if (brick_instance.position.y >= placement or gml.collision_point_test(brick_instance.position.x, brick_instance.position.y+16, 'brick', 0, 0) or gml.collision_point_test(brick_instance.position.x, brick_instance.position.y+16, 'hard_block', 0, 0)):  brick_instance.down = true
		if (gml.collision_point_test(brick_instance.position.x-16, brick_instance.position.y, 'brick', 0, 0) or gml.collision_point_test(brick_instance.position.x-16, brick_instance.position.y, 'hard_block', 0, 0)):  brick_instance.left = true
		if (gml.collision_point_test(brick_instance.position.x+16, brick_instance.position.y, 'brick', 0, 0) or gml.collision_point_test(brick_instance.position.x+16, brick_instance.position.y, 'hard_block', 0, 0)):  brick_instance.right = true
	...
	for lush_instance in all_lushes:
		...
		if (lush_instance.position.y == 0 or gml.collision_point_test(lush_instance.position.x, lush_instance.position.y-16, 'lush', 0, 0)):  lush_instance.up = true
		if (lush_instance.position.y >= placement or gml.collision_point_test(lush_instance.position.x, lush_instance.position.y+16, 'lush', 0, 0)):  lush_instance.down = true
		if (gml.collision_point_test(lush_instance.position.x-16, lush_instance.position.y, 'lush', 0, 0)):  lush_instance.left = true
		if (gml.collision_point_test(lush_instance.position.x+16, lush_instance.position.y, 'lush', 0, 0)):  lush_instance.right = true
		...
		if (not lush_instance.down):
			...
			if (not gml.collision_point_test(lush_instance.position.x, lush_instance.position.y+16, 'solid', 0, 0) and global.graphics_high):
	...
	for dark_instance in all_darks:
		...
		if (dark_instance.position.y == 0 or gml.collision_point_test(dark_instance.position.x, dark_instance.position.y-16, 'dark', 0, 0)):  dark_instance.up = true
		if (dark_instance.position.y >= placement or gml.collision_point_test(dark_instance.position.x, dark_instance.position.y+16, 'dark', 0, 0)):  dark_instance.down = true
		if (gml.collision_point_test(dark_instance.position.x-16, dark_instance.position.y, 'dark', 0, 0)):  dark_instance.left = true
		if (gml.collision_point_test(dark_instance.position.x+16, dark_instance.position.y, 'dark', 0, 0)):  dark_instance.right = true
	...
	for ice_instance in all_ice:
		...
		if (gml.collision_point_test(ice_instance.position.x, ice_instance.position.y-16, 'ice', 0, 0)):  ice_instance.up = true
		if (gml.collision_point_test(ice_instance.position.x, ice_instance.position.y+16, 'ice', 0, 0)):  ice_instance.down = true
		if (gml.collision_point_test(ice_instance.position.x-16, ice_instance.position.y, 'ice', 0, 0)):  ice_instance.left = true
		if (gml.collision_point_test(ice_instance.position.x+16, ice_instance.position.y, 'ice', 0, 0)):  ice_instance.right = true
		...
		if (not ice_instance.down):
			...
			if (randi_range(1,20) == 1 and not gml.collision_point_test(ice_instance.position.x, ice_instance.position.y+16, 'solid', 0, 0)): gml.instance_create(ice_instance.position.x, ice_instance.position.y+16, 'ice_bottom')
	...
	for temple_instance in all_temples:
		...
		if (temple_instance.position.y == 0 or gml.collision_point_test(temple_instance.position.x, temple_instance.position.y-16, 'temple', 0, 0) or gml.collision_point_test(temple_instance.position.x, temple_instance.position.y+16, 'temple_fake', 0, 0)):  temple_instance.up = true
		if (temple_instance.position.y >= placement or gml.collision_point_test(temple_instance.position.x, temple_instance.position.y+16, 'temple', 0, 0) or gml.collision_point_test(temple_instance.position.x, temple_instance.position.y+16, 'temple_fake', 0, 0)):  temple_instance.down = true
		if (gml.collision_point_test(temple_instance.position.x-16, temple_instance.position.y, 'temple', 0, 0) or gml.collision_point_test(temple_instance.position.x-16, temple_instance.position.y, 'temple_fake', 0, 0)):  temple_instance.left = true
		if (gml.collision_point_test(temple_instance.position.x+16, temple_instance.position.y, 'temple', 0, 0) or gml.collision_point_test(temple_instance.position.x+16, temple_instance.position.y, 'temple_fake', 0, 0)):  temple_instance.right = true
	...
	for temple_fake_instance in all_fake_temples:
		...
		if (temple_fake_instance.position.y == 0 or gml.collision_point_test(temple_fake_instance.position.x, temple_fake_instance.position.y-16, 'temple', 0, 0) or gml.collision_point_test(temple_fake_instance.position.x, temple_fake_instance.position.y-16, 'temple_fake', 0, 0)):  temple_fake_instance.up = true
		if (gml.collision_point_test(temple_fake_instance.position.x, temple_fake_instance.position.y+16, 'temple', 0, 0) or gml.collision_point_test(temple_fake_instance.position.x, temple_fake_instance.position.y+16, 'temple_fake', 0, 0)):  temple_fake_instance.down = true
		if (gml.collision_point_test(temple_fake_instance.position.x-16, temple_fake_instance.position.y, 'temple', 0, 0) or gml.collision_point_test(temple_fake_instance.position.x-16, temple_fake_instance.position.y, 'temple_fake', 0, 0)):  temple_fake_instance.left = true
		if (gml.collision_point_test(temple_fake_instance.position.x+16, temple_fake_instance.position.y, 'temple', 0, 0) or gml.collision_point_test(temple_fake_instance.position.x+16, temple_fake_instance.position.y, 'temple_fake', 0, 0)):  temple_fake_instance.right = true
	...
	for water_instance in all_water:
		...
		if (water_instance.type == "Water"):
			...
			if (gml.collision_point_test(water_instance.position.x, water_instance.position.y-16, 'water', 0, 0)):  water_instance.up_water = true
			if (gml.collision_point_test(water_instance.position.x, water_instance.position.y-16, 'solid', 0, 0)):  water_instance.up = true
			if (gml.collision_point_test(water_instance.position.x, water_instance.position.y+16, 'solid', 0, 0) and not gml.collision_point_test(water_instance.position.x, water_instance.position.y+16, water, 0, 0)):  water_instance.down = true
			...
			if (water_instance.up_water and gml.collision_point_test(water_instance.position.x, water_instance.position.y-32, 'water', 0, 0) and water_instance.down and randi_range(1,4) == 1):
	...
	for lava_instance in all_lava:
		...
		if (gml.collision_point_test(lava_instance.position.x, lava_instance.position.y-16, 'water', 0, 0)):  lava_instance.up_water = true
		if (gml.collision_point_test(lava_instance.position.x, lava_instance.position.y-16, 'solid', 0, 0)):  lava_instance.up = true
		if (gml.collision_point_test(lava_instance.position.x, lava_instance.position.y+16, 'solid', 0, 0)):  lava_instance.down = true
		...
		#/*
		#if (up_water and gml.collision_point_test(x, y-32, water, 0, 0) and down and randi_range(1,4) == 1):
		#
	...
	for vine_instance in all_vines:
		...
		if (gml.collision_point_test(vine_instance.position.x+8, vine_instance.position.y-8, 'ladder', 0, 0)):  vine_instance.up = true
		if (gml.collision_point_test(vine_instance.position.x+8, vine_instance.position.y+16, 'ladder', 0, 0)):  vine_instance.down = true
	...
	for block_instance in all_blocks:
		...
		if (gml.collision_point_test(block_instance.position.x, block_instance.position.y+16, 'brick', 0, 0) or gml.collision_point_test(block_instance.position.x, block_instance.position.y+16, 'temple', 0, 0) or gml.collision_point_test(block_instance.position.x, block_instance.position.y+16, 'hard_block', 0, 0)):  block_instance.down = true

		# don't want push blocks next to lava until we tighten block_instance.up liquid draining
		if (gml.collision_point_test(block_instance.position.x-16, block_instance.position.y, 'lava', 0, 0) or gml.collision_point_test(block_instance.position.x+16, block_instance.position.y, 'lava', 0, 0)): block_instance.down = false
	...
	for tree_instance in all_trees:
		...
		if (gml.collision_point_test(tree_instance.position.x, tree_instance.position.y-16, 'tree', 0, 0)):  tree_instance.up = true
		if (gml.collision_point_test(tree_instance.position.x, tree_instance.position.y+16, 'tree', 0, 0)):  tree_instance.down = true
		if (gml.collision_point_test(tree_instance.position.x-16, tree_instance.position.y, 'tree', 0, 0)):  tree_instance.left = true
		if (gml.collision_point_test(tree_instance.position.x+16, tree_instance.position.y, 'tree', 0, 0)):  tree_instance.right = true
	...
	for tree_branch_instance in all_tree_branches:
		...
		if (gml.collision_point_test(tree_branch_instance.position.x, tree_branch_instance.position.y-16, 'leaves', 0, 0)):  tree_branch_instance.up = true
		if (gml.collision_point_test(tree_branch_instance.position.x, tree_branch_instance.position.y+16, 'tree_branch', 0, 0)):  tree_branch_instance.down = true
		if (gml.collision_point_test(tree_branch_instance.position.x-16, tree_branch_instance.position.y, 'tree_branch', 0, 0)):  tree_branch_instance.left = true
		if (gml.collision_point_test(tree_branch_instance.position.x+16, tree_branch_instance.position.y, 'tree', 0, 0)):  tree_branch_instance.right = true
