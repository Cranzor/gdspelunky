...
func gm_loop(): #---[FLAG] consider running every event in the same order as step
	if !paused:
		...
		if GameSettings.smooth_motion: View.smooth_scroll_camera_vertically = false
		...
		get_tree().call_group("gm_object", "reset_animation_history") #--- see sprite_index in gm_object
