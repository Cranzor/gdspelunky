...
func create():
	...
	if (gml.room_get_name() != "intro"):
		...
		if (n < 20): sprite_index = "dark_gold"
			...
			if (randi_range(1,40) == 1): gml.instance_create(position.x+8, position.y+8, Objects.sapphire_big, self)
			elif (randi_range(1,60) == 1): gml.instance_create(position.x+8, position.y+8, Objects.emerald_big, self)
			elif (randi_range(1,80) == 1): gml.instance_create(position.x+8, position.y+8, Objects.ruby_big, self)
