	...
	if true:
		...
		if (global.shake > 0):
			...
			if (global.shake_toggle or gml.view("yview") <= 0):
				...
				#gml.view_yview += 3 #--- this value is now based on screen shake setting. 3 is set as the default value
				gml.view_yview += GameSettings.get_screen_shake_value()
				global.shake_toggle = false
				GameSettings.start_controller_rumble() #--- adding this here to have rumble whenever the screen shakes
