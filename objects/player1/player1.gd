...
func handle_bomb_arrows() -> void:
	if (hold_arrow == ARROW_BOMB):
		...
		if (in_level.is_in_shop(position.x, position.y)):
...
func handle_player_dead_or_stunned() -> void:
	...
	if (dead or stunned):
		...
		if (k_down and (platform_character_is(ON_GROUND) or state == HANGING) and not k_right and not k_left):
			...
			else: View.vertical_scroll(4) #gml.view_yview += 4 #--- changed this to allow for interpolated scrolling
	...
	if (InLevel.is_level() and active and k_pay_pressed and not dead and not stunned):
		...
		if (in_level.is_in_shop(position.x, position.y) and gml.instance_exists("shopkeeper")):
			...
			if (shopkeeper.style == "kissing" and gml.distance_to_object(damsel, self) < 16):
				...
				if (global.thief_level > 0 or global.murderer or not obj.for_sale):
				...
				elif (n == 0 and global.money >= in_level.get_kiss_value()):
					...
					if (obj.for_sale and not obj.held):
						...
						global.money -= in_level.get_kiss_value()
						global.plife += 1
					...
					if (n == 0): global.message = "YOU NEED $" + str(in_level.get_kiss_value()) + "!"
...
func character_draw_event() -> void:
	...
	if (blink_toggle != 1):
		...
		if ((state == CLIMBING or (sprite_index == "p_exit" or sprite_index == "damsel_exit" or sprite_index == "tunnel_exit")) and global.has_jetpack and not whipping):
			...
			#gml.draw_sprite("jetpack_left",-1,position.x+4,position.y-1,self) #--- commenting out draw_sprite to use DrawSprite class
		...
		if (facing == RIGHT):
			...
			if (hold_arrow == ARROW_NORM):
				...
				arrow_draw.draw_sprite(Vector2(4, 1), "arrow_right")
