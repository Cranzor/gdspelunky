...
func handle_player_active():
	if (active):
		...
		elif (platform_character_is(ON_GROUND) and fall_timer > 16 and not gml.collision_rectangle(position.x-8, position.y-8, position.x+8, position.y+8, 'spring_trap', 0, false, self)): # LONG DROP
...
func bomb_rope_and_whipping_handling(): #--- Also handles picking up items and attacking with items
	if (dead or stunned or not active):
				...
				if (position.x < obj.position.x):
					...
					if (not gml.collision_point(position.x+8, position.y, "solid", 0, false)):
						...
						if (not  gml.collision_rectangle(obj.position.x-8, obj.position.y, obj.position.x-7, obj.position.y+16, 'solid', 0, false, self)):
							position.x -= 8
						elif (not  gml.collision_rectangle(obj.position.x+7, obj.position.y, obj.position.x+8, obj.position.y+16, 'solid', 0, false, self)):
							obj.position.x += 8
						else: obj.t = false
					...
					if (not  gml.collision_rectangle(obj.position.x+7, obj.position.y, obj.position.x+8, obj.position.y+16, 'solid', 0, false, self)):
						obj.position.x += 8
					elif (not  gml.collision_rectangle(obj.position.x-8, obj.position.y, obj.position.x-7, obj.position.y+16, 'solid', 0, false, self)):
						obj.position.x -= 8
					else: obj.t = false
		...
		if (k_attack_pressed and state != DUCKING and state != DUCKTOHANG and not whipping and
			...
			elif (gml.collision_rectangle(position.x-8, position.y, position.x+8, position.y+8, 'enemy', 0, false, self)):
...
func handle_hit_by_arrow():
	if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8,  position.y+8, "arrow", 0, false, self)):
...
func handle_hit_by_rock():
	if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8,  position.y+8, "rock", 0, false, self)):
...
func handle_hit_by_laser():
	if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8,  position.y+8, "laser", 0, false, self)):
...
func handle_hit_by_psychic_wave():
	if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8,  position.y+8, "psychic_wave", 0, false, self) and not stunned and not dead):
...
func handle_hit_by_explosion():
	if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8,  position.y+8, "explosion", 0, false, self)):
...
func handle_hit_by_spears_left():
	var obj = gml.collision_rectangle(position.x-6, position.y-6, position.x+6,  position.y+7, "spears_left", 0, false, self) #instance_nearest(position.x, position.y, spears_left) ---[FLAG] doesn't seem to be used for anything but come back and check this
	if (obj):
...
func handle_hit_by_smash_trap():
	if (gml.collision_rectangle(position.x-6, position.y-6, position.x+6,  position.y+7, "smash_trap", 0, false, self)):
...
func handle_hit_by_ceiling_trap():
	var obj = gml.collision_rectangle(position.x-2, position.y-9, position.x+2,  position.y-7, "ceiling_trap", 0, false, self) #instance_nearest(position.x, position.y-8, ceiling_trap)
	if (obj):
...
func handle_spike_collision():
	col_spikes = false
	if (gml.collision_rectangle(position.x-4, position.y-4, position.x+4,  position.y+8, "spikes", 0, false, self)): col_spikes = true
...
func collect_arrow():
	if (hold_item):
		...
		if (hold_item.type == "bow"):
			...
			if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8,  position.y+8, "arrow", 0, false, self) and not dead and not stunned):
...
func collect_treasure():
	if (gml.collision_rectangle(position.x-8, position.y-8, position.x+8, position.y+8, "treasure", 0, false, self) and not dead and not stunned):
...
#
	...
	if (hold_item.sprite_index == "bomb"):
		...
		if (k_down):
			...
			var n = 0
			while (gml.collision_rectangle(tx-4, ty-4, tx+4,  ty+4, "solid", 0, false, self) and n < 3 and ty > 16):
