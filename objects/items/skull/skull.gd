...
func destroy():
	...
	if (break_pieces):
		...
		for i in gml.rand(1,2):
	
			var piece = gml.instance_create(position.x-2, position.y-2, Objects.bone)
...
func step():
	...
	if (held):
		...
		if (col_top and y_vel < 0):
			...
			if (y_vel < 2): to_destroy = true
			y_vel = -y_vel * 0.8
	
	
		if (col_left or col_right):
	
			if (abs(x_vel) > 2): to_destroy = true
			x_vel = -x_vel * 0.5
	...
	if (gml.collision_rectangle(position.x-3, position.y-3, position.x+3,  position.y+3, "enemy", 0, false, self) and (abs(x_vel) > 2 or abs(y_vel) > 2)):
	...
	if (type != "damsel" and gml.collision_rectangle(position.x-3, position.y-3, position.x+3,  position.y+3, "damsel", 0, false, self) and (abs(x_vel) > 2 or abs(y_vel) > 2)):
