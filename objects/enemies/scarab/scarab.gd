...
func step():
	if ((position.x > gml.view("xview")-20 and position.x < gml.view("xview") + gml.view("wview")+4 and
		...
		dir = 0
		var character = gml.get_instance("character") #---[FLAG] may have to adjust this for multiplayer
		var dist = gml.point_distance(position.x+8, position.y+8, character.position.x, character.position.y)
		...
		if (status == IDLE):
			...
			if (counter == 0 and x_vel < 1 and y_vel < 1):
				...
				if (dist < 64):
					...
					dir = gml.point_direction(position.x+8, position.y+8, character.position.x, character.position.y) + 180
			
				else:
			
					dir = gml.rand(0,360)
			
				x_vel = 4 * cos(gml.degtorad(dir))
				y_vel = -4 * sin(gml.degtorad(dir))
				counter = gml.rand(10,30)
		
		
			if (Collision.is_collision_right(1, self) and x_vel > 0):
		
				x_vel = -x_vel
		
			if (Collision.is_collision_left(1, self) and x_vel < 0):
			...
			if (Collision.is_collision_bottom(1, self) and y_vel > 0):
