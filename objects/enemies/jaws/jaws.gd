	...
	if ((position.x > gml.view("xview")-48 and position.x < gml.view("xview") + gml.view("wview")+48 and
		...
		if (status == IDLE):
			...
			if (dir == 0): # DY:  right
				...
				elif (not gml.collision_rectangle(position.x-32, position.y, position.x,  position.y+32, "solid", 0, null)):
				...
				elif (not gml.collision_rectangle(position.x+16, position.y, position.x+48,  position.y+32, "solid", 0, null)):
			...
			if (player1.swimming and not player1.dead):
				...
				if (player1.position.x < position.x+8):
					...
					if (sprite_index == "jaws_right" and not gml.collision_rectangle(position.x-32, position.y, position.x,  position.y+32, "solid", 0, null)):
