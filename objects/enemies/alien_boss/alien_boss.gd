...
func animation_end():
	if (sprite_index == "alien_boss_die"):
		...
		var all_barrier_emitters = gml.get_all_instances("barrier_emitter")
		for barrier_emitter_instance in all_barrier_emitters:
			gml.instance_destroy(barrier_emitter_instance)
