...
func step():
	...
	dist = gml.point_distance(position.x+8, position.y+8, character.position.x, character.position.y)
	...
	if (status == IDLE):
		...
		dir = gml.point_direction(position.x+8, position.y+8, character.position.x, character.position.y)
		position.x += 1 * cos(gml.degtorad(dir))
		position.y += -1 * sin(gml.degtorad(dir))
