...
func step():
	...
	if (not invincible):
		...
		if (gml.collision_rectangle(position.x-3, position.y-3, position.x+3,  position.y+3, "spears_left", 0, null) and
	...
	if (held):
		...
		if (Collision.is_collision_bottom(2, self) or gml.collision_rectangle(position.x-4, position.y-6, position.x+4,  position.y+8, "web", 0, null)):
