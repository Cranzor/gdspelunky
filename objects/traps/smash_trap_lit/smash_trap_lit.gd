...
func step():
	if (position.x > gml.view("xview")-16 and position.x < gml.view("xview") + gml.view("wview")+16 and
		...
		if (status == IDLE):
			...
			if (gml.collision_rectangle(position.x-1,position.y-1,position.x+17,position.y+17,"tomb_lord",0,0, self)): hit = true
		...
		if (gml.collision_rectangle(position.x+1, position.y+1, position.x+15,  position.y+15, "lava", 0, 0, self)):
