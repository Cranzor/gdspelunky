	...
	if (true):
		...
		if (other.invincible or abs(x_vel) < 1):
			...
			if (abs(x_vel) < 1 or other.position.y > position.y+13):
				...
				var tile = gml.instance_position(other.position.x+8, other.position.y-1, "spikes")
				if (tile): gml.instance_destroy(tile)
...
func step():
	# action_inherited
	#super() #--- commenting out as this seems to do nothing, since moving_solid has no step function
	...
	if (true):
		...
		if (not gml.collision_point(position.x, position.y+16, "solid", 0, 0)):
			...
			col_left = gml.collision_rectangle(position.x-16, position.y-16, position.x-8,  position.y+16, "solid", false, true, self)
			col_right = gml.collision_rectangle(position.x+8, position.y-16, position.x+16,  position.y+16, "solid", false, true, self)
